class StateSearchesController < ApplicationController
  ...
  def create
    if params[:state_search][:state_id].blank? ||
      ...
      @results = Person.find(:all, :conditions => {:state_id => params[:state_search][:state_id]}) #TODO: Replace state find with actual search criteria
      ...
      if @results.empty?
        ...
        respond_to do |format|
          ...
          format.csv {
            ...
            csv = csv_data.to_s
            render :text => csv
            flash[:notice] = "#{count} employees found."
          }
        end
      end
    end
