    ...
    { Date.parse('2009-01-05') => '20090105',
      ...
      context "with a date of #{date}" do
        it "should return #{str}" do
         Format.date_yyyymmdd(date).should == str
        end
      end
    end
  end

  describe '#left_fixed' do
    { ['Sam', 10, nil] => 'Sam       ',
      ['Sam', 9, '0'] => 'Sam000000',
      ['', 10, nil] => '          ',
      ['', 9, '0'] => '000000000',
      ...
      context "with a value of #{data}" do
        it "should return #{returned_value}" do
          if filler.nil?
            result = Format.left_fixed(data, fixed_length)
            result.should == returned_value
          else
            result = Format.left_fixed(data, fixed_length, filler)
            result.should == returned_value
          end
  ...
  describe '#number_right_fixed' do
    { [7, 10, nil] => '         7',
      ...
      context "with a value of #{data}" do
        it "should return #{returned_value}" do
          if filler.nil?
            result = Format.number_right_fixed(data, fixed_length)
            result.should == returned_value
          else
            result = Format.number_right_fixed(data, fixed_length, filler)
            result.should == returned_value
          end
        end
      end
